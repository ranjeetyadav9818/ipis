...
public class TestIntensityController  {
	...
	public void testGetAutoIntensity() {
		...
		ResponseEntity<String> response = restTemplate.exchange("/intensity/Auto",
				HttpMethod.GET, entity, String.class);
		Assert.assertNotNull(response.getBody());
	}
    
    @Test
    public void testSaveAutoIntensity() {
    	
    	int id = 0;
    	Intensity intensity = restTemplate.getForObject("/intensity/" + id, Intensity.class);
    	restTemplate.put("/setmessages/" + id, intensity);
		Intensity updatedIntensity = restTemplate.getForObject("/intensity/" + id, Intensity.class);
		Assert.assertNotNull(updatedIntensity);
