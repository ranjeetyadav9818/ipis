...
@Component
public class MldbUtil implements UtilityInterface {
	...
	public void sendCommand(Device device, String command) {
		...
		if (command.equals(Command.Start.toString())) {
			...
			try {
				...
				byte[] packetData = tcpPacket.getCommandPacket(DeviceType.mldb.toString(), Command.Start.toString(),
						ipAddress);
			...
			try {
				...
				byte[] packetData = tcpPacket.getCommandPacket(DeviceType.mldb.toString(), Command.Stop.toString(),
						ipAddress);
			...
			try {
				...
				byte[] packetData = tcpPacket.getCommandPacket(DeviceType.mldb.toString(), Command.SoftReset.toString(),
						ipAddress);
			...
			try {
				...
				byte[] packetData = tcpPacket.getCommandPacket(DeviceType.mldb.toString(),
	...
	// To set auto intensity (in interface set configuration)
	@Override
	public void sendConfiguration(List<Device> device, int intensity, int intDisplayTimeMldb) {
	...
	public void sendPackage(String ipAddress, int intensity, int intDisplayTimeMldb) {

		try {
			System.out.println("send package");

			byte[] configData = this.getConfigData(intensity, intDisplayTimeMldb);

			byte[] packetData = tcpPacket.getConfigDataPacket(DeviceType.mldb.toString(),
	...
	private void defaultDataPacket(String ipAddress, String data, String speed, String effect, String letterSize,
			int gap,int timeDelay) {
...
//			System.out.println();
             ...
             if(enableDiable.get(0).getMldb().equals("enable"))
             {
             ...
             {
            	 throw new HandledException("mldb devices are disabled","Disabled");
		...
		try {
			...
			System.out.println();
             ...
             if(enableDiable.get(0).getMldb().equals("enable"))
             {
                  ...
                  byte[] packetData = tcpPacket.getConfigDataPacket(DeviceType.mldb.toString(), Command.MessageDataPacket.toString(), ipAddress, configData);     
	...
	private void tadbDataPacket(String ipAddress, List<OnlineTrain> onlineTrain, String speed, String effect,
		...
		//String status=enableDiable.get(0).getMldb();
		...
		for (int i = 0; i < onlineTrain.size(); i++) {
		 ...
		 try {
             ...
             if(enableDiable.get(0).getMldb().equals("enable"))
             {
                  ...
                  byte[] packetData = tcpPacket.getConfigDataPacket(DeviceType.mldb.toString(), Command.DataTransfer.toString(), ipAddress, configData); 
	...
	private  byte[] getTadbPacket(short leftColumn, short rightColumn, short topRow, short bottomRow, String trainNo, String time,
			String ad, String platform, String trainName,String trainStatus,String speedValue, String effect, String letterSizeValue,
			int gap,int timeDelay) throws Exception {

		List<Byte> dataPacket = new ArrayList<Byte>();
		...
		{
			dataPacket.add(leftColumnBytes[i]);	
		}
		...
		for(int i=0;i<leftColumnBytes.length;i++)
		{
			dataPacket.add(rightColumnBytes[i]);
			
		}
		
		topRowBytes=dataUtil.getDataOnTwoBytesParam(topRow);

		for(int i=0;i<leftColumnBytes.length;i++)
		{
			dataPacket.add(topRowBytes[i]);
			
		}

		bottomRowBytes=dataUtil.getDataOnTwoBytesParam(bottomRow);

		for(int i=0;i<leftColumnBytes.length;i++)
		{
			dataPacket.add(bottomRowBytes[i]);
		...
		dataPacket.add(letter8);

		//int timeDelay = 0x00;

		dataPacket.add((byte) timeDelay);
		...
		{
			dataPacket.add(startAddressBytes[i]);
			
		}
		
		for (int i = 2; i < trainNoArray.length; i++) {
			dataPacket.add((byte) trainNoArray[i]);
		}
